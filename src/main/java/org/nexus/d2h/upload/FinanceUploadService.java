...
@Slf4j
@Service
public class FinanceUploadService {

    private static final int BATCH_SIZE = 100;
    private static final List<String> REQUIRED_HEADERS = List.of(
            "retailer_code", "transaction_date", "transaction_type", "amount", "reference"
    );
    ...
    @Transactional
    public FinanceUploadResult upload(MultipartFile file) {
        if (file.isEmpty()) {
    ...
    private FinanceUploadResult processCsv(MultipartFile file) {
        ...
        BigDecimal[] amounts = {BigDecimal.ZERO, BigDecimal.ZERO}; // processed, failed
        ...
        try (CSVReader reader = new CSVReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            while ((row = reader.readNext()) != null) {
                ...
                try {
                    ...
                    financeService.createFromUpload(rd.retailer(), rd.type(), rd.date(),
                    ...
                    amounts[1] = amounts[1].add(safeAmount(row, idx));
                }
            }
    ...
    private FinanceUploadResult processExcel(MultipartFile file) {
        Tenant tenant = resolveTenant();
        List<UploadResult.RowError> errors = new ArrayList<>();
        Set<String> seenRefs = new HashSet<>();
        int[] counters = {0, 0, 0};
        BigDecimal[] amounts = {BigDecimal.ZERO, BigDecimal.ZERO};
    ...
    private void publishUploadEvent(Tenant tenant, int total, int success, int failed,
        ...
        try {
            ...
            eventPublisher.publish(NotificationEventType.FINANCE_UPLOAD_COMPLETED,
                    "upload", payload);
        } catch (Exception e) {
    ...
    private RowData parseRow(String[] row, Map<String, Integer> idx, int rowNum,
        ...
        String dateStr = require(cell(row, idx, "transaction_date"), "transaction_date", rowNum);
        String typeStr = require(cell(row, idx, "transaction_type"), "transaction_type", rowNum);
        ...
        TransactionType type = parseType(typeStr, rowNum);
        ...
        return new RowData(retailer, type, date, amount, pm, reference,
    ...
    private RowData parseExcelRow(Row row, Map<String, Integer> idx, int rowNum,
        ...
        String dateStr = require(excelCell(row, idx, "transaction_date"), "transaction_date", rowNum);
        String typeStr = require(excelCell(row, idx, "transaction_type"), "transaction_type", rowNum);
    ...
    private record RowData(Retailer retailer, TransactionType type, LocalDate date,
                           BigDecimal amount, PaymentMethod paymentMethod, String reference,
                           String paymentReference, String description, String remarks) {}
    ...
    private TransactionType parseType(String s, int rowNum) {
        try { return TransactionType.valueOf(s.trim().toUpperCase()); }
        catch (IllegalArgumentException e) {
            throw new BusinessException("INVALID_TYPE", "Row " + rowNum + ": Invalid transaction type: " + s);
        }
    }
    ...
    private BigDecimal safeAmount(String[] row, Map<String, Integer> idx) {
        try { return new BigDecimal(cell(row, idx, "amount").trim()); }
