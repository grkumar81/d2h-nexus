-- V3__asset_schema.sql
-- STB asset inventory, history, sales and sale items.
-- Tenant isolation via tenant_id. Optimistic locking via version column.
...
CREATE INDEX idx_assets_tenant_status   ON stb_assets (tenant_id, status);
CREATE INDEX idx_assets_tenant_retailer ON stb_assets (tenant_id, retailer_id);
CREATE INDEX idx_assets_tenant_serial   ON stb_assets (tenant_id, serial_number);
...
CREATE INDEX idx_asset_history_tenant ON stb_asset_history (tenant_id, changed_at);
...
CREATE INDEX idx_sales_tenant_retailer ON stb_sales (tenant_id, retailer_id);
CREATE INDEX idx_sales_tenant_date     ON stb_sales (tenant_id, transaction_date);
